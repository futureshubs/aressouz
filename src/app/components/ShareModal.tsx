...
export function ShareModal({ isOpen, onClose, placeId, placeName, placeImage }: ShareModalProps) {
  ...
  const generateShareLink = async () => {
    ...
    try {
      ...
      const publicAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Indub25kbXFtdXZqdWdib215b2x6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzcxNDIzMjYsImV4cCI6MjA1MjcxODMyNn0.yvW3YI8fXo_fZl1C9n_SkdHFgqv7nW2hk-H1emQI2Zk';

      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-27d0d16c/places/${placeId}/share`,
        {
          method: 'POST',
          headers: {
      ...
      setError(err.message || 'Link yaratishda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Modal ochilganda va tabga qaytganda havolani yangilash
  useEffect(() => {
    if (!isOpen) return;
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className="bg-[#0a0a0a] rounded-3xl shadow-2xl overflow-hidden border border-white/10">
              {/* Header */}
              <div className="relative p-6 pb-4 border-b border-white/10">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">Ulashish</h3>
                    <p className="text-sm text-white/60 mt-0.5">Joyni do'stlaringiz bilan ulashing</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-base font-semibold text-white truncate">{placeName}</h4>
                    <p className="text-sm text-white/60">Aresso.app</p>
                  </div>
                </div>
                ...
                {loading && (
                  <div className="text-center py-4">
                    ...
                    <p className="text-white/60 mt-2">Link yaratilmoqda...</p>
                  </div>
                )}
                ...
                {shareUrl && !loading && (
                  <>
                    <div className="space-y-3">
                      <label className="text-sm font-medium text-white/80">Ulashish linki:</label>
                    ...
                    <div className="space-y-3">
                      <label className="text-sm font-medium text-white/80">Ijtimoiy tarmoqlarda:</label>
                    ...
                    <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                      <p className="text-xs text-white/40 mb-1">Ulashish kodi:</p>
