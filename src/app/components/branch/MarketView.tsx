...
export default function MarketView({ branchId, readOnly = false }: MarketViewProps) {
  ...
  const [marketVisibilityTick, setMarketVisibilityTick] = useState(0);
  useVisibilityRefetch(() => setMarketVisibilityTick((t) => t + 1));
  ...
  }, [branchId, loadPendingCashMarketOrders, marketVisibilityTick]);
  ...
  }, [branchId, loadPendingCashMarketOrders]);
    ...
    <div className="space-y-6">
      {getStoredBranchToken() &&
        ...
        <div
          ...
          style={{
            background: isDark
              ? 'linear-gradient(145deg, rgba(251, 191, 36, 0.12), rgba(255, 255, 255, 0.03))'
              : 'linear-gradient(145deg, #fffbeb, #ffffff)',
            borderColor: isDark ? 'rgba(251, 191, 36, 0.35)' : 'rgba(251, 191, 36, 0.5)',
          }}
        >
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              <h3 className="text-lg font-bold flex items-center gap-2">
                ...
                Onlayn market — naqd to‘lov (filial qabuli)
              </h3>
              <p
                className="text-sm mt-1"
                style={{ color: isDark ? 'rgba(255, 255, 255, 0.65)' : 'rgba(0, 0, 0, 0.55)' }}
              >
                Mijoz ilovadan naqd tanlasa, buyurtma avval shu yerda ko‘rinadi. «Qabul qilish»dan keyin tayyorlovchi
                paneliga tushadi. Click / Payme kabi onlayn to‘lovlar darhol tayyorlovchiga boradi.
              </p>
            </div>
            {loadingCashPending && (
              <Loader2 className="w-6 h-6 animate-spin shrink-0" style={{ color: accentColor.color }} />
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm font-semibold">
            <span
              ...
              style={{
                background: isDark ? 'rgba(245, 158, 11, 0.2)' : 'rgba(245, 158, 11, 0.18)',
                color: isDark ? '#fbbf24' : '#b45309',
              }}
            >
              Kutilayotgan: {pendingCashMarketOrders.length}
            </span>
            <span
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full"
              style={{
          ...
          <div className="space-y-3">
            <p className="text-sm font-bold" style={{ color: isDark ? '#fbbf24' : '#b45309' }}>
              Filial qabulini kutayotgan naqd buyurtmalar
            </p>
            {!loadingCashPending && pendingCashMarketOrders.length === 0 ? (
              ...
              >
                Hozir kutilayotgan naqd market buyurtmasi yo‘q
              </p>
            ) : null}
            {pendingCashMarketOrders.map((ord) => (
              ...
              >
                  ...
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="text-sm opacity-80">
                      ...
                      {ord.deliveryZone ? (
                        <>
                          {' · '}
                          Zona: <span className="font-medium">{String(ord.deliveryZone)}</span>
                        </>
                      ) : null}
                    </p>
                    {ord.addressText ? (
                      <p className="text-sm opacity-75 line-clamp-2">Manzil: {ord.addressText}</p>
                    ) : null}
                    {ord.notes ? (
                      <p className="text-sm opacity-75 line-clamp-2">Izoh: {ord.notes}</p>
                    ) : null}
                  </div>
                  {canEdit && (
                    <button
                      ...
                      className="shrink-0 px-4 py-2.5 rounded-xl font-semibold text-white transition active:scale-95 disabled:opacity-60"
                      style={{
                        background: accentColor.gradient,
                        boxShadow: `0 4px 14px ${accentColor.color}35`,
                      }}
                    >
                ...
                <div
                  className="text-xs space-y-1 opacity-80 border-t pt-3"
                  ...
                  {(Array.isArray(ord.items) ? ord.items : []).slice(0, 12).map((it: any, idx: number) => (
                    <div key={idx} className="flex justify-between gap-2">
                      ...
                      <span className="shrink-0">
                        {(Number(it.price || 0) * Number(it.quantity || 1)).toLocaleString('uz-UZ')} so‘m
                      </span>
                    </div>
                  ))}
                  {Array.isArray(ord.items) && ord.items.length > 12 ? (
                    <p className="italic">+ yana {ord.items.length - 12} pozitsiya</p>
                  ) : null}
                </div>
              </div>
            ))}
          ...
          {cancelledCashMarketOrders.length > 0 ? (
            ...
            >
              <p className="text-sm font-bold flex items-center gap-2" style={{ color: '#ef4444' }}>
                <XCircle className="w-4 h-4 shrink-0" />
                Bekor qilingan naqd market buyurtmalari ({cancelledCashMarketOrders.length})
              </p>
              <p
                className="text-xs -mt-1"
                style={{ color: isDark ? 'rgba(255,255,255,0.55)' : 'rgba(0,0,0,0.5)' }}
              >
                Oxirgi 50 ta bekor buyurtma ko‘rsatiladi (filialdagi market, naqd to‘lov).
              </p>
                  ...
                  >
                    ...
                    {ord.updatedAt || ord.createdAt ? (
                      <p className="text-xs opacity-60">
                        {ord.updatedAt
                          ? `Yangilangan: ${new Date(ord.updatedAt).toLocaleString('uz-UZ')}`
                          : `Yaratilgan: ${new Date(ord.createdAt).toLocaleString('uz-UZ')}`}
                      </p>
                    ) : null}
                    {ord.addressText ? (
                      <p className="text-sm opacity-75 line-clamp-2">Manzil: {ord.addressText}</p>
                    ...
                    <div
                      className="text-xs space-y-1 opacity-80 border-t pt-2"
                      style={{ borderColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)' }}
                    >
                      {(Array.isArray(ord.items) ? ord.items : []).slice(0, 8).map((it: any, idx: number) => (
                        <div key={idx} className="flex justify-between gap-2">
                          <span className="truncate">
                            {it.name || it.title || 'Mahsulot'}
                            {it.variantName || it.size ? ` (${it.variantName || it.size})` : ''} ×
                      ...
                      {Array.isArray(ord.items) && ord.items.length > 8 ? (
                        <p className="italic">+ yana {ord.items.length - 8} pozitsiya</p>
                      ) : null}
                    </div>
                  </div>
                ))}
