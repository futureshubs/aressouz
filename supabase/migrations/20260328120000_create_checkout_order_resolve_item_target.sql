-- order_items_one_target: num_nonnulls(product_variant_id, listing_id) = 1
-- Resolve default product_variant_id when client sends product_id only (e.g. web cart hash id).
    ...
    loop
      ...
      if v_ins_pvid is null and v_ins_lid is null and v_ins_pid is not null then
        ...
        where pv.product_id = v_ins_pid
          and pv.status = 'active'::product_status
      ...
      if v_ins_pvid is null and v_ins_lid is null then
        raise exception 'order_items: need listing_id, product_variant_id, or product_id with an active variant';
      end if;
