...
import {
  ...
  getOrderKeys,
  persistKvOrder,
} from "./services/order-kv-lookup.ts";
import { registerPaymeReceiptRoutes } from "./routes/payme-receipt-routes.ts";
import * as chatMsg from "./chat-messages.ts";
import { registerPromoRoutes } from "./routes/promo-routes.ts";
import {
  CHAT_KEY_PREFIX,
  CHAT_MESSAGE_KEY_PREFIX,
  buildChatId,
} from "./chat-constants.ts";
import { registerChatRoutes, userChatsHandler } from "./routes/chat-routes.ts";
import { registerKvSyncRoutes } from "./routes/kv-sync-routes.ts";
...
registerKvSyncRoutes(app, {
  validateAccessToken,
  validateAdminSession: validateAdminAccess,
});
...
app.post("/make-server-27d0d16c/orders/:orderId/confirm-receipt", async (c) => {
  try {
    ...
    const { sync: receiptSync } = await persistKvOrder(
      updatedOrder as Record<string, unknown>,
      orderRecord.key,
    );
    const persistedPrimary = updatedOrder;
    if (!receiptSync.synced) {
      console.warn('[confirm-receipt] relational sync skipped:', orderId);
    }
