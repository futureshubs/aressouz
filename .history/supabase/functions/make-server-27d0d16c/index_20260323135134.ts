...
import * as payme from "./payme.tsx";
import * as atmos from "./atmos.tsx";
import preparersRoutes from "./preparers.tsx";
import twoFactorRoutes from "./two-factor.tsx";
import relationalRoutes from "./relational-routes.ts";
import { cacheKeyFromParts, getOrSetCached, paginateItems, readPagination, setCached } from "../_shared/performance.ts";
...
// Authentication middleware - bypass JWT validation for public endpoints
app.use('*', async (c, next) => {
  ...
  const publicEndpoints = [
    ...
    '/make-server-27d0d16c/public/branches',
    '/make-server-27d0d16c/public/branches/location',
  ...
  if (isPublic) {
    console.log(`🔓 PUBLIC ENDPOINT: ${path} - Skipping auth`);
    await next();
    return;
  }
  
  // TEMPORARY: Allow all requests during development
  console.warn(`⚠️ DEV MODE: Allowing request to ${path} without authentication`);
  await next();
});
...
app.options('*', (c) => {
  ...
  return c.text('OK', 200, {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-Access-Token, x-access-token, X-Seller-Token, x-seller-token, X-Admin-Code, x-admin-code, X-Request-ID, x-request-id, apikey',
...
async function validateAdminAccess(c: any) {
  ...
  if (adminCode && getConfiguredAdminCodes().includes(adminCode)) {
    return { success: true, mode: 'code', userId: null };
  }
...
const getOrderKeys = (orderId: string) => [`order:${orderId}`, `order:market:${orderId}`];
...
app.get("/make-server-27d0d16c/products", async (c) => {
  try {
    ...
    const pagination = readPagination(c.req.raw, { limit: 120, offset: 0 });

    const [marketProducts, shopProducts] = await Promise.all([
      getOrSetCached('kv:product:all', CACHE_TTL.products, () => kv.getByPrefix('product:')),
      getOrSetCached('kv:shop_product:all', CACHE_TTL.products, () => kv.getByPrefix('shop_product:')),
    ]);
    ...
    const paginated = paginateItems(allProducts, pagination.limit, pagination.offset);
...
app.get("/make-server-27d0d16c/branch-products", async (c) => {
  try {
    ...
    let products = await getOrSetCached('kv:branchproduct:all', CACHE_TTL.products, () => kv.getByPrefix('branchproduct:'));
    ...
    if (regionId || districtId) {
      const branches = await getOrSetCached('kv:branch:all', CACHE_TTL.branches, () => kv.getByPrefix('branch:'));
    ...
    const paginated = paginateItems(products, pagination.limit, pagination.offset);
    return c.json({
      products: paginated.items,
      meta: {
        total: paginated.total,
...
app.get("/make-server-27d0d16c/public/branches", async (c) => {
  try {
    const pagination = readPagination(c.req.raw, { limit: 200, offset: 0 });
    const branches = await getOrSetCached('kv:branch:all', CACHE_TTL.branches, () => kv.getByPrefix('branch:'));
    const paginated = paginateItems(branches, pagination.limit, pagination.offset);
...
app.get("/make-server-27d0d16c/public/branches/location", async (c) => {
  try {
    ...
    const paginated = paginateItems(filteredBranches, pagination.limit, pagination.offset);
    return c.json({
      branches: paginated.items,
      meta: {
        total: paginated.total,
...
app.get("/make-server-27d0d16c/homepage-data", async (c) => {
  try {
    const region = c.req.query('region');
    const district = c.req.query('district');
    const cacheKey = cacheKeyFromParts('homepage-data', region, district);

    const payload = await getOrSetCached(cacheKey, CACHE_TTL.homepage, async () => {
      const [branches, categories, products] = await Promise.all([
        getOrSetCached('kv:branch:all', CACHE_TTL.branches, () => kv.getByPrefix('branch:')),
        getOrSetCached('kv:categories:all', CACHE_TTL.categories, () => kv.get('categories')),
        getOrSetCached('kv:product:all', CACHE_TTL.products, () => kv.getByPrefix('product:')),
      ]);

      const visibleBranches = branches
        .filter((branch: any) => {
          if (region && branch.regionId !== region) return false;
          if (district && branch.districtId !== district) return false;
          return true;
        })
        .slice(0, 12);
    ...
    console.log('Homepage data error:', error);
    return c.json({ error: 'Bosh sahifa ma\'lumotlarini olishda xatolik' }, 500);
  }
});
...
app.post("/make-server-27d0d16c/payment-methods", async (c) => {
  try {
    ...
    const methodData = {
      ...
      isTestMode: isTestMode !== false, // Default to test mode
...
// Payme: Create transaction
app.post("/make-server-27d0d16c/payments/payme/create", async (c) => {
  try {
    const { amount, orderId, userId } = await c.req.json();
    
    console.log('💳 Creating Payme transaction:', { amount, orderId, userId });
    
    // Get Payme configuration
    ...
    const transaction = {
      ...
      method: 'payme',
      status: 'pending',
      isTestMode: paymeConfig.isTestMode,
      createdAt: new Date().toISOString(),
    };
    
    await kv.set(`transaction:${transactionId}`, transaction);
    
    console.log(`✅ Payme transaction created: ${transactionId}`);
    return c.json({ 
      success: true,
      transaction,
      paymentUrl: paymeConfig.isTestMode 
        ? `https://test.paycom.uz/${paymeConfig.config.merchantId}`
        : `https://checkout.paycom.uz/${paymeConfig.config.merchantId}`,
    });
  } catch (error: any) {
...
app.post("/make-server-27d0d16c/payments/click/create", async (c) => {
  try {
    ...
    const transaction = {
      ...
      method: 'click',
      status: 'pending',
      isTestMode: clickConfig.isTestMode,
      createdAt: new Date().toISOString(),
    };
    ...
    return c.json({ 
      success: true,
      transaction,
      paymentUrl: clickConfig.isTestMode 
        ? `https://test.click.uz/services/pay?service_id=${clickConfig.config.serviceId}&merchant_id=${clickConfig.config.merchantId}&transaction_param=${transactionId}`
        : `https://my.click.uz/services/pay?service_id=${clickConfig.config.serviceId}&merchant_id=${clickConfig.config.merchantId}&transaction_param=${transactionId}`,
    });
  } catch (error: any) {
...
app.get("/make-server-27d0d16c/shops", async (c) => {
  try {
    ...
    const district = c.req.query('district');
    const pagination = readPagination(c.req.raw, { limit: 120, offset: 0 });
    const shops = await getOrSetCached('kv:shop:all', CACHE_TTL.shops, () => kv.getByPrefix('shop:'));
    ...
    const paginated = paginateItems(activeShops, pagination.limit, pagination.offset);
    return c.json({
      success: true,
      shops: paginated.items,
      meta: {
        total: paginated.total,
        limit: paginated.limit,
        offset: paginated.offset,
...
app.post('/make-server-27d0d16c/payme/create-receipt', async (c) => {
  try {
    const { amount, orderId, items, phone } = await c.req.json();
    
    console.log('💳 Creating Payme receipt:', { amount, orderId, items, phone });
    ...
    if (!payme.isPaymeConfigured()) {
      return c.json({ error: 'Payme sozlanmagan' }, 500);
    }
    
    const result = await payme.createReceipt(amount, orderId, items, phone);
...
app.post('/make-server-27d0d16c/payme/check-receipt', async (c) => {
  try {
    ...
    const result = await payme.checkReceipt(receiptId);
...
app.post('/make-server-27d0d16c/payme/get-receipt', async (c) => {
  try {
    ...
    const result = await payme.getReceipt(receiptId);
...
app.post('/make-server-27d0d16c/payme/cancel-receipt', async (c) => {
  try {
    ...
    const result = await payme.cancelReceipt(receiptId);
...
app.post('/make-server-27d0d16c/atmos/create-transaction', async (c) => {
  try {
    const { amount, orderId, customerPhone, customerName } = await c.req.json();
    
    console.log('💳 Creating Atmos transaction:', { amount, orderId, customerPhone, customerName });
    
    if (!amount || !orderId || !customerPhone) {
      return c.json({ error: 'Amount, orderId va customerPhone majburiy' }, 400);
    }
    
    if (!atmos.isAtmosConfigured()) {
      return c.json({ error: 'Atmos sozlanmagan' }, 500);
    }
    
    const result = await atmos.createTransaction(amount, orderId, customerPhone, customerName);
    
    if (!result.success) {
      return c.json({ error: result.error || 'Tranzaksiya yaratishda xatolik' }, 400);
    }
...
app.post('/make-server-27d0d16c/atmos/check-transaction', async (c) => {
  try {
    ...
    const result = await atmos.checkTransaction(transactionId);
...
app.post('/make-server-27d0d16c/atmos/cancel-transaction', async (c) => {
  try {
    ...
    const result = await atmos.cancelTransaction(transactionId);
...
app.post("/make-server-27d0d16c/orders", async (c) => {
  try {
    ...
    const normalizedPaymentMethod = String(data.paymentMethod || 'cash');
    ...
    const order = {
      ...
      paymentMethod: normalizedPaymentMethod, // cash, click, payme, atmos
      paymentStatus,
      paymentRequiresVerification: ONLINE_PAYMENT_METHODS.has(normalizedPaymentMethod),
      ...
      statusHistory: [{
        ...
        timestamp: new Date().toISOString(),
        note: ONLINE_PAYMENT_METHODS.has(normalizedPaymentMethod)
...
app.post('/make-server-27d0d16c/orders/cancel', async (c) => {
  try {
    ...
    if (orderRecord.order.status === 'delivered') {
...
app.get("/make-server-27d0d16c/categories", async (c) => {
  try {
    let categories = await getOrSetCached('kv:categories:all', CACHE_TTL.categories, () => kv.get('categories'));
    ...
    if (!categories) {
      ...
      setCached('kv:categories:all', categories, CACHE_TTL.categories);
